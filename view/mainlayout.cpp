...

{
    ...
    searchBar->setValidator(new QRegularExpressionValidator(QRegularExpression("[A-Z0-9a-z<>=, ]{0,50}/i")));//aggiungere il . se si vuole usare la rierca dell'efficenza elettrico su windows
    carbNecessarioBar->setValidator(new QRegularExpressionValidator(QRegularExpression("[0-9]{0,50}/i")));
    kmPerBar->setValidator(new QRegularExpressionValidator(QRegularExpression("[0-9]{0,50}/i")));
